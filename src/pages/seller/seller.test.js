  ...
  db: {
    collection: jest.fn(() => ({
      ...
      getDocs: jest.fn(() => Promise.resolve({
        docs: [
          { data: () => ({ id: 1, title: "Product 1", price: 10, imageUrl: "https://example.com/product1.jpg" }) },
          { data: () => ({ id: 2, title: "Product 2", price: 20, imageUrl: "https://example.com/product2.jpg" }) },
          { data: () => ({ id: 3, title: "Product 3", price: 30, imageUrl: "https://example.com/product3.jpg" }) }
        ]
      }))
    })),
  },
  storage: {
    ref: jest.fn(() => ({
      put: jest.fn(() => Promise.resolve({
        ref: {
          getDownloadURL: jest.fn(() => Promise.resolve("https://example.com/product1.jpg"))
        }
      }))
    }))
  },
...
describe('Seller', () => {
  ...
  test('adds a new product', async () => {
    ...
    // Fill in the form
    fireEvent.change(screen.getByPlaceholderText('Product title...'), { target: { value: 'New Product' } });
    fireEvent.change(screen.getByPlaceholderText('Product price...'), { target: { value: '40' } });
  ...
  test("submit product with no title should show error message", async () => {
    ...
    fireEvent.change(inputFile, { target: { files: [new File(['file contents'], 'test.png', { type: 'image/png' })] } });
    fireEvent.click(submitButton);

    await waitFor(() => expect(getByText("Please enter a product title.")).toBeInTheDocument());
  });
